...
export async function getBooks(libraryLocation = null) {
  if (!libraryLocation) {
    const [mainRows, hsRows] = await batchRead([SHEETS.BOOKS_MAIN, SHEETS.BOOKS_HS])
    const combined = [...mainRows, ...hsRows]
    cache.set('books', combined, BOOKS_TTL)
    return combined
  }
  const sheetName = libraryLocation === LIBRARY.HIGH_SCHOOL ? SHEETS.BOOKS_HS : SHEETS.BOOKS_MAIN
...
export function toBook(r) {
  return {
    ...
    year:          r[COL.BOOKS.YEAR],       // renamed from isbn
...
const createSchema = Joi.object({
  ...
  year:          Joi.string().max(10).optional().allow(''),  // renamed from isbn
...
async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    if (search) {
      ...
      rows = rows.filter(r =>
        ...
        r[COL.BOOKS.YEAR]?.includes(q)                 // search by year too
      )
    }
  ...
  if (req.method === 'POST') {
    ...
    const sheetName = value.location === LIBRARY.HIGH_SCHOOL ? SHEETS.BOOKS_HS : SHEETS.BOOKS_MAIN
    const id  = uuid()
    const now = new Date().toISOString()

    await appendRow(sheetName, [
      id, value.title, value.author,
      value.year || '',          // col 3 — was isbn, now year
      value.category, value.location,
      value.totalCopies, value.totalCopies,
      value.description || '', value.coverImageUrl || '',
      now, req.user.id
    ])
