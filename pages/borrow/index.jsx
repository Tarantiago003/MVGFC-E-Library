...
const LIBRARIES = [
  { id: 'HIGH_SCHOOL',  name: 'High School Department', description: 'Library for HS students and faculty.', icon: '🏫' },
  { id: 'MAIN_LIBRARY', name: 'Main Library', description: 'Central academic library for all college departments.', icon: '🏛️' }
]

export default function BorrowPage() {
  const router = useRouter()
  const [step,       setStep]   = useState(0)
  const [library,    setLib]    = useState(null)
  const [book,       setBook]   = useState(null)
  const [search,     setSearch] = useState('')
  const [toast,      setToast]  = useState(null)
  const [submitting, setSub]    = useState(false)
  const [confirmImgErr, setConfirmImgErr] = useState(false)

  const { books, loading: bLoading } = useBooks({ location: library, search })
  const { submitBorrow }             = useBorrows()

  // Reset confirm image error when book changes
  function selectBook(b) {
    setBook(b)
    setConfirmImgErr(false)
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setToast({ message: 'Borrow request submitted! Awaiting approval.', type: 'success' })
  ...
  const confirmCover = toCoverUrl(book?.coverImageUrl)
        ...
        <div>
          ...
          <div className="bg-white rounded-2xl border border-green-100 shadow-card p-5 mb-5">
            <div className="flex gap-4">
              {confirmCover && !confirmImgErr
                ? <img
                    src={confirmCover}
                    alt={book.title}
                    onError={() => setConfirmImgErr(true)}
                    className="w-16 h-[88px] object-cover rounded-xl flex-shrink-0 bg-green-50"
                  />
                : <div className="w-16 h-[88px] bg-green-100 rounded-xl flex items-center
              ...
              <div>
                ...
                <span className="mt-2 inline-block bg-green-100 text-green-700 text-xs
